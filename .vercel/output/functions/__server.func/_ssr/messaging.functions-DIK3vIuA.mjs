...
import { g as getSiteUrl, s as sendTransactionalEmailServer } from "./site-url-Du7GW4Cs.mjs";
...
async function senderDisplayName(senderId) {
  const [{
    ...
    data: profile
  }] = await Promise.all([supabaseAdmin.from("organiser_profiles").select("org_name").eq("user_id", senderId).maybeSingle(), supabaseAdmin.from("sponsor_profiles").select("brand_name").eq("user_id", senderId).maybeSingle(), supabaseAdmin.from("referral_partner_profiles").select("full_name").eq("user_id", senderId).maybeSingle(), supabaseAdmin.from("profiles").select("email").eq("id", senderId).maybeSingle()]);
  return org?.org_name ?? sp?.brand_name ?? ref?.full_name ?? profile?.email?.split("@")[0] ?? "Someone";
}
...
const sendMessage = createServerFn({
  ...
  const [{
    data: recipient
  }, senderName] = await Promise.all([supabaseAdmin.from("profiles").select("email").eq("id", data.recipient_id).maybeSingle(), senderDisplayName(userId)]);
  if (recipient?.email) {
