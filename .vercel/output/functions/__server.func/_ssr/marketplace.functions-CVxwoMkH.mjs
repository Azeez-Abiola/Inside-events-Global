...
import { a as supabasePublic, s as supabaseAdmin } from "./client.server-k0C2Btf0.mjs";
...
const getMarketplaceFilterOptions = createServerFn({
  ...
  try {
    let q = supabasePublic.from("events").select("event_type, primary_sector, country, format, ige_vetted, decision_makers_pct").in("status", ["approved", "listed"]);
    ...
    const [{
      ...
      data: allLive,
      error: allErr
    }] = await Promise.all([q, supabasePublic.from("events").select("ige_vetted, decision_makers_pct").in("status", ["approved", "listed"])]);
...
const listMarketplaceEvents = createServerFn({
  ...
  try {
    let q = supabasePublic.from("events").select("id, slug, name, event_type, format, start_date, end_date, city, country, primary_sector, attendance_size, decision_makers_pct, banner_image_url, ige_vetted, currency, created_at", {
      count: "exact"
    ...
    if (ids.length) {
      const {
        data: tiers
      } = await supabasePublic.from("event_sponsorship_tiers").select("event_id, price, currency").in("event_id", ids).order("price", {
        ascending: true
      });
...
const getPublicEventBySlug = createServerFn({
  ...
  try {
    const {
      data: ev,
      error
    } = await supabasePublic.from("events").select("*").eq("slug", data.slug).in("status", ["approved", "listed"]).maybeSingle();
    if (error) throw new Error(error.message);
    if (!ev) return {
      event: null,
      tiers: [],
    ...
    const [{
      ...
      data: organiser
    }] = await Promise.all([supabasePublic.from("event_sponsorship_tiers").select("*").eq("event_id", ev.id).order("display_order"), ev.organiser_id ? supabasePublic.from("organiser_profiles").select("org_name, logo_url, bio, website, track_record, event_history, past_sponsor_logos").eq("user_id", ev.organiser_id).maybeSingle() : Promise.resolve({
      data: null
    })]);
    return {
      event: ev,
...
const submitCommitmentForm = createServerFn({
  ...
  try {
    const {
      getRequestHeader
    } = await import("./server-BI-XTmyt.mjs");
