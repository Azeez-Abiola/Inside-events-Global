...
const getOrganiserPipeline = createServerFn({
  ...
  let deals = [];
  if (eventIds.length) {
    const [f, d] = await Promise.all([supabaseAdmin.from("commitment_forms").select("id, event_id, company_name, contact_name, currency, budget_range_min, budget_range_max, tier_id, submitted_at, referral_partner_id").in("event_id", eventIds).order("submitted_at", {
      ascending: false
...
const getSponsorDashboard = createServerFn({
  ...
  if (ids.length) {
    const {
      data: evs
    } = await supabaseAdmin.from("events").select("id, slug, name, banner_image_url, start_date, city, country").in("id", ids);
