...
import {
  ...
  listMarketplaceEventsForSponsor,
...
export function MarketplaceBrowser({
  ...
  return (
    <div className="space-y-6">
      <div ref={enableReveal ? (headerRef as any) : undefined}>
        <div
          {...(enableReveal ? { "data-reveal": true } : {})}
        ...
        >
          ...
          <div className="flex items-center gap-2">
            ...
            <button
              ...
              className="md:hidden inline-flex items-center gap-1.5 rounded-md border border-border bg-card px-3 py-2 text-sm"
            >
        ...
        <div
          {...(enableReveal ? { "data-reveal": true, "data-delay": "1" } : {})}
        ...
        {activeChips.length > 0 && (
          <div
            {...(enableReveal ? { "data-reveal": true, "data-delay": "2" } : {})}
          ...
          >
            {activeChips.map((c) => (
              <button
                ...
                type="button"
                onClick={c.onRemove}
                className="inline-flex items-center gap-1 rounded-full bg-brand-soft px-3 py-1 text-xs font-medium text-primary-deep hover:bg-primary/20"
              >
      ...
      <div className="grid grid-cols-1 gap-8 md:grid-cols-[260px_1fr]">
        <aside
          className={`${open ? "fixed inset-0 z-50 overflow-auto bg-background p-6 md:relative md:p-0" : "hidden"} md:block`}
        >
              ...
              {availableEventTypes.length > 0 && (
                <FilterGroup label="Event type">
                  <MultiSelect
                    ...
                    onChange={(v) => toggle("event_types", v)}
                  />
                </FilterGroup>
              )}
                  ...
                  {availableFormats.map((f) => (
                    <label key={f} className="flex items-center gap-2 py-1 text-sm">
                      <input
                        ...
                        onChange={() => toggle("format", f as typeof filters.format)}
                      />
                ...
                <FilterGroup label="Quality">
                  {(facets?.has_vetted_events || facets?.has_non_vetted_events) && (
                    <label className="flex items-center gap-2 py-1 text-sm">
                      <input
                        type="checkbox"
                        checked={filters.vetted_only}
                        onChange={(e) => toggle("vetted_only", e.target.checked)}
                      />
                      IGE vetted only
                    </label>
                  )}
                  {facets?.has_decision_maker_events && (
                    <label className="flex items-center gap-2 py-1 text-sm">
                      <input
                        type="checkbox"
                        checked={filters.decision_makers}
                        onChange={(e) => toggle("decision_makers", e.target.checked)}
                      />
        ...
        <div ref={enableReveal ? (gridRef as any) : undefined}>
          {isLoading ? (
            ...
            <div className="grid grid-cols-1 gap-5 sm:grid-cols-2 lg:grid-cols-3">
              {events.map((e: any, i: number) => (
                <div
                  key={e.id}
                  {...(enableReveal
                    ? { "data-reveal": true, "data-delay": String(Math.min((i % 3) + 1, 3)) }
                    : {})}
                >
                  <MarketplaceEventCard event={e} />
                </div>
              ))}
            </div>
          )}
...
function MarketplaceEventCard({ event }: { event: any }) {
  ...
  return (
    <Link
      ...
      className="group block h-full overflow-hidden rounded-xl border border-border bg-card transition-all hover:-translate-y-0.5 hover:shadow-lg"
        ...
        {event.starting && (
          <div className="mt-3 border-t border-border pt-2 text-sm">
            ...
            <span className="font-display font-bold">
              {fmtDual(event.starting.currency, Number(event.starting.price), ratesData?.rates)}
            </span>
          </div>
        )}
      </div>
